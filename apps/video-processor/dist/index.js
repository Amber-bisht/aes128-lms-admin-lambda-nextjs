...
const handler = async (event) => {
    ...
    try {
        ...
        const result = await (0, processor_1.processVideo)(key, videoId);
        console.log("Processing complete. Updating DB...", result);
        // 3. Update Main API (or DB directly) with Keys
        // Yes, VI and Hex Key are different for EACH video because processVideo generates them fresh.
        const apiResponse = await fetch(`http://localhost:4000/admin/courses/lectures/${videoId}/complete`, {
            method: 'POST',
            headers: {
